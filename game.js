...
function main() {
    ...
    var app = new App(canvas, output, banner, playAgain);
    ...
    playAgain.onfocus = function () { playAgain.blur(); }
    playAgain.onclick = function () { document.location.reload(); }
 
    window.requestAnimationFrame(update);
}

function App(canvas, output, banner, playAgain) {
    this.playing = true;
    ...
    function generateEntities() {
        ...
        function randcoor(dimLength) {
            return Math.floor(Math.random() * dimLength);
        }
        
        for (var i = 0; i < Math.min(symbols.length - 1, descriptions.length, 20); i++) {
            var rx = randcoor(canvas.width - 20);
            var ry = randcoor(canvas.height - 20);
            items.push(new Item(symbols[i + 1][0], symbols[i + 1][1], descriptions[i], new Vector(rx, ry)));
        }
        
        var entities = {
            robot: new Robot(new Vector(300, 300)),
            kitten: new Item(symbols[0][0], symbols[0][1], 'KITTEN!', new Vector(randcoor(canvas.width - 20), randcoor(canvas.height - 20))),
            items: items
        };
